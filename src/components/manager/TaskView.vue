<template>
  <div class="bg-white dark:bg-dark-grey rounded-lg py-3">
    <div class="p-5 pr-3 flex flex-col gap-2 max-h-[90vh] overflow-y-auto ">
      ...
      <div>
        <p class="text-medium-grey dark:text-white text-xs font-bold pb-4">Subtasks <span class="text-main-blue">({{ subtasksCompleted }} completed)</span></p>
        <div class="flex flex-col gap-2">
          <Subtask v-for="(subtask, index) in boardsStore?.getTask?.subtasks" :key="index" :subtask="subtask" />
        </div>
      </div>
      <div class="mt-4">
        <p class="text-medium-grey dark:text-white text-xs font-bold pb-4 -mb-2">Current Status</p>
