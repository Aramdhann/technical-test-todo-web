<template>
  <div ref="select" @focusin="onFocusSelect" tabindex="1" class="text-sm text-black dark:text-white font-bold rounded-md px-4 py-2
  ...
  hover:border-main-blue focus:border-main-blue group">
    ...
    <div ref="options"
      class="hidden absolute rounded left-0 top-full mt-1 w-full bg-white dark:bg-dark-grey group-focus:block">
      <div @click="onClickOption(index, column.name)"
        class="px-4 py-2 text-medium-grey rounded-lg hover:text-white hover:bg-main-blue" v-for="(column, index) in store.getColumns">
        {{ column.name }}
      </div>
    </div>
  </div>
